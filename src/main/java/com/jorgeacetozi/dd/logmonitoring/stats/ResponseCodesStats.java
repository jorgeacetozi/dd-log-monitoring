...
public class ResponseCodesStats extends Stats<Map<String, Long>> {

  public ResponseCodesStats(Storage storage, int pastSeconds) {
  ...
  public Map<String, Long> calculate() {
    return storage.getDataPoints(pastSeconds).stream()
        ...
        .collect(Collectors.groupingBy(req -> "HTTP " + req.getStatus(), Collectors.counting()))
        .entrySet()
        .stream()
  ...
  public Optional<String> calculateAndReturnAsString() {
    ...
    return Optional.of(new StringBuilder()
      .append("                 TOP 5 HTTP RESPONSE CODES")
      .append(newLine)
      .append("                 -------------------------")
      .append(newLine)
