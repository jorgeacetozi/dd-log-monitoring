...
public class SectionStatsTest {

  @Test
  public void shouldCalculateTheTopFiveAccessedSections() {
    Storage storage = new InMemoryStorage(5);
    Stats<Map<String, Long>> sectionStats = new SectionsStats(storage, 5);
    ...
    Map<String, Long> topFiveSections = sectionStats.calculate();
    List<String> keys = new ArrayList<>(topFiveSections.keySet());

    assertThat(keys.get(0), equalTo("/api"));
    assertThat(keys.get(1), equalTo("/report"));
